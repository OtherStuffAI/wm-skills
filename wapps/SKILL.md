...
Use this skill when an agent needs to build or work with a Wingman WApp: a business-specific web app that owns its UI, business rules, and data model while using Wingman Autopilot for AI pipelines, agent sessions, tasks, and code execution.
...
- local UI and routes;
- an app-owned database or a Tower-managed isolated Postgres namespace;
...
For a stateless WApp hosted separately from Tower, Tower can provide the app's
Postgres persistence without app-specific Tower code. Keep a thin WApp server
between browsers and Tower, and read
[`references/tower-app-database.md`](references/tower-app-database.md) before
designing or operating this pattern.

## Wingman Dev Operations

In a managed Wingman environment, WApps and Flight Deck are managed through
Wingman/Autopilot, not by starting standalone localhost previews from the app
repo. Use the Wingman CLI from the Autopilot repo for runtime/app/version
operations:
...
For Flight Deck workspace operations, prefer the PG-native CLI surface as it is
implemented:
...
Do not restart Autopilot, Flight Deck, Tower, this WApp, or another registered WApp unless the user explicitly approves that restart in the current conversation. Report a pending restart and smoke test when approval is absent.

If the user references a number such as `1256` while looking at the running app,
treat it as a Wingman-served app version, not a port, unless they explicitly say
it is a port.
